...
class BookSourceViewModel(application: Application) : BaseViewModel(application) {
    ...
    /**
     * 分类导出：直接把指定的一批书源写成文件（不参与“导出全部”的比例判断）
     */
    fun savePartsToFile(
        ...
        saveToFile(sources.toBookSource(), name, success)
    }
    ...
    /**
     * 书源去重：地址（忽略 `#备注`）、名称、搜索/目录/正文规则都相同的视为同一个源，
     * 保留最早导入的那一个，返回其余重复项。
     */
    fun scanDuplicates(onResult: (List<BookSource>) -> Unit) {
        execute {
            findDuplicates()
        }.onSuccess {
            onResult(it)
    ...
    fun deleteSources(sources: List<BookSource>) {
    ...
    private fun findDuplicates(): List<BookSource> {
        val all = appDb.bookSourceDao.all.sortedBy { it.customOrder }
        val seen = HashSet<String>()
        val duplicates = ArrayList<BookSource>()
        all.forEach { source ->
            val key = buildString {
                append(source.bookSourceName).append('|')
                append(source.bookSourceUrl.substringBefore('#')).append('|')
                append(source.searchUrl).append('|')
                append(source.getSearchRule()).append('|')
                append(source.getTocRule()).append('|')
                append(source.getContentRule())
            }
            if (!seen.add(key)) {
